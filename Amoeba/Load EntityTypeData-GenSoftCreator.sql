...
FROM AmoebaDB.dbo.Entities where id in (SELECT DISTINCT  ApplicationEntities.EntityId
											...
											WHERE        (ApplicationEntities.ApplicationId = @appId))
...
BEGIN 
	...
	if(@entityName = 'EntityType') set @entityName = @entityName + 'Where applicationid = ' + (select id from [GenSoft-Creator].dbo.Applications where dbname = @DBName) 
	--if(@entityName = 'DomainProcess') @entityName = @entityName + 'Where applicationid = ' + (select id from [GenSoft-Creator].dbo.Applications where dbname = @DBName) 
	PRINT @entityName
