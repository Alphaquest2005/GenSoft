...
declare @attributeId int, @datatypeId int, @Counter int = 0
			...
			set @attributeId = (select id from [Attributes] where Name = 'Id') 

		if(@attributeId is null)
		begin
			set @datatypeId = (select id from [Type] where [Name] = 'int32')
			if(@datatypeId is not null)
			begin
				
					insert into Attributes(DataTypeId, [Name]) values (@datatypeId, 'Id')
					set @attributeId = (select id from Attributes where [name] = 'Id')
	...
	delete from [Type] where id in (SELECT        Type.Id
								FROM            Type INNER JOIN
														 DBType ON Type.Id = DBType.Id)
	...
	BEGIN 
		...
		Insert into [Type] ([Name]) values (@TableName)
		set @entityId = null
		set @entityId = (select id from [Type] where [Name] = @TableName)

		if @entityId is null
				break;
		
		Insert into [Type] ([Name]) values ('r'+@TableName)
		set @rEntityId = null
		set @rEntityId = (select id from [Type] where [Name] = 'r'+@TableName)

		
		insert into EntityType(ID, EntitySetName) values (@entityId, @EntitySet)
		insert into DomainEntityType(ID) values (@entityId)

		insert into EntityType(ID, EntitySetName) values (@rEntityId, 'r'+@EntitySet)
		insert into DomainEntityType(ID) values (@rEntityId)

		insert into DBType (ID,TableName, SchemaName) values (@EntityId,@TableName, @schema)
		insert into EntityId(ID) values (@attributeId)
		...
		FROM            INFORMATION_SCHEMA.COLUMNS INNER JOIN
								 ...
								 DataType INNER JOIN
								 Type ON DataType.Id = Type.Id ON Type.Name LIKE '%' + AmoebaDB.dbo.DataTypes.Name + '%' LEFT OUTER JOIN
								 Attributes AS Attributes_1 ON INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME = Attributes_1.Name
		...
		INSERT INTO EntityTypeAttributes
		...
		set @nameCount = (SELECT        COUNT(Attributes.Name) AS Expr1
						FROM            EntityTypeAttributes INNER JOIN
												 ...
												 EntityType ON EntityTypeAttributes.EntityTypeId = EntityType.Id INNER JOIN
		...
		begin
			...
			insert into EntityViewModelCommands(EntityViewModelId,ViewModelCommandId) Values(@entityViewModelId, 2)
			insert into EntityViewModelCommands(EntityViewModelId,ViewModelCommandId) Values(@entityViewModelId, 3)
